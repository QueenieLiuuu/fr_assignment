...
# create database, use the cloud database
def create_db():
    db = connect(
        host="mongodb+srv://qingmin:Liu8888@cluster0.vzvxo.mongodb.net/myFirstDatabase?retryWrites=true&w=majority")
    return db
...
def reset_db():
    ...
    for i in range(0, len(books)):
        data = Book(bookid=books[i].get('id'), name=books[i].get('name'), author=books[i].get('author'),
                    check_status='N', br='')
        data.save()
...
def display_books_status():
    ...
    for data in Book.objects():
        ...
        res = res + f"{data.bookid, data.name, data.author, data.check_status, data.br, br_name}"
...
def process_action(action, borrower_id="", book_id=""):
    if not Borrower.objects(userid=borrower_id):
        res = f"Borrower with Id {borrower_id} does not exist"
        return res

    if not Book.objects(bookid=book_id):
        res = f"Book with Id {book_id} does not exist"
        return res
    ...
    if action == "return":
        if not (book_rec.br == borrower_id):
            res = f'{borrower_rec.name, book_rec.name} has not currently checked out'
        else:
            book_rec.check_status = 'N'
            book_rec.br = ''
            book_rec.save()
            res = f'{borrower_rec.name} has returned {book_rec.name}'
        return res

    if action == "checkout":
        if book_rec.check_status == 'Y':
            res = f'{book_rec.name} is already checked out by someone.'
        else:
            book_rec.chk_status = 'Y'
            book_rec.br = borrower_id
            book_rec.save()
            res = f'{borrower_rec.name, book_rec.name}has checked out'
        return res
